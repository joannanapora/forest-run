...
function getSteps() {
  return [
    ...
    "Meeting place and Event description",
    "Upload Event Image",
  ];
}
...
const CreateEvent = ({ history }) => {
  ...
  const [imgid, setImageId]: [
    string,
    Dispatch<SetStateAction<string>>
  ] = useState("");
  const [alert, setAlert]: [
    Notifications,
    Dispatch<SetStateAction<Notifications>>
  ] = useState({
  ...
  const [allDetails, setAllDetails]: [
    IAllEventDetails,
    Dispatch<SetStateAction<IAllEventDetails>>
  ] = useState({
    organizerName: "",
  ...
  const [createEvent] = useMutation(CREATE_EVENT, {
    ...
    onError: (error) => {
      ...
      if ((error.graphQLErrors[0].message as any).statusCode === 400) {
        if (
          (error.graphQLErrors[0].message as any).message === "date-in-past"
        ) {
  ...
  const handleDetailChange = (prop) => (
    ...
    if (prop === "image") {
      postImage(event.target.files[0]).then((result) => {
    ...
    if (prop === "distance" && event.target.value.length > 5) {
      setAllDetails({
        ...allDetails,
        distance: event.target.value.slice(0, -1),
      });
    }
    ...
    if (prop === "distance" && Number(event.target.value) < 0) {
      setAllDetails({
        ...allDetails,
        distance: (Number(event.target.value) * -1).toString(),
      });
    } else {
  ...
  const handleValidation = (): boolean => {
    if (allDetails.when === "") {
      setAlert({
        ...alert,
        missedInputs: { ...alert.missedInputs, emptyWhen: true },
      });
      return false;
    }
    if (timeValue === null) {
      setAlert({
        ...alert,
        missedInputs: { ...alert.missedInputs, emptyTime: true },
      });
      return false;
    }
    if (allDetails.location === "") {
      setAlert({
        ...alert,
        missedInputs: { ...alert.missedInputs, emptyLocation: true },
      });
      return false;
    }
    if (allDetails.distance === "") {
      setAlert({
        ...alert,
        missedInputs: { ...alert.missedInputs, emptyDistance: true },
      });
      return false;
    }
    if (dateValue === null && allDetails.when === "one time event") {
      setAlert({
        ...alert,
        missedInputs: { ...alert.missedInputs, emptyDate: true },
      });
      return false;
    }
      ...
      case 1:
        return (
          <MeetingPoint
            ...
            changeOrganizerPhoneNumber={handleDetailChange(
              "organizerPhoneNumber"
            )}
  ...
  const handleSubmit = () => {
    createEvent({
      variables: {
        ...
        ...(mapOptionsToWhen(allDetails.when) === 0 && {
